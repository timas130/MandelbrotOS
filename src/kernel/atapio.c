...
void ata_wait_bsy(pio_bus_t *device) {
  while (inb(device->base_port + PIO_PORT_STATUS) & PIO_STATUS_BSY);
}
bool ata_wait_drq(pio_bus_t *device) {
  while (true) {
    uint8_t status = inb(device->base_port + PIO_PORT_STATUS);
    if (status & PIO_STATUS_DRQ) return true;
    if (status & PIO_STATUS_ERR ||
        status & PIO_STATUS_DF) return false;
  }
}

void ata_pio_select_drive(pio_bus_t *device, bool slave) {
...
void ata_pio_send_lba48(pio_bus_t *device, uint64_t lba,
                        uint16_t sectors) {
